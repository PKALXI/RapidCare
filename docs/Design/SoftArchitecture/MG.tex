...
\begin{description}
\item[\refstepcounter{acnum} \actheacnum \label{acLogin}:] The application should support different authentication methods, such as password-based, face-id, biometric login etc.
...
\item[\refstepcounter{acnum} \actheacnum \label{acrealtime}:] The application shall enable real-time streaming of audio input to written text to ensure immediate transcription without delays.   
\end{description}
...
\begin{description}
\item[\refstepcounter{ucnum} \uctheucnum \label{ucSecure}:] The requirement to secure patient's profile ensures confidentiality will remain a constant priority in the app.
\item[\refstepcounter{ucnum} \uctheucnum \label{ucCompatible}:] The ability of the app to be compatible with the latest versions of different operating systems such as Windows, Linux and macOS will remain a constant requirement.
...
\item[\refstepcounter{ucnum} \uctheucnum \label{uceaseofuse}:] The app's ease of use is anticipated to remain a constant requirement to allow them to focus on patient's care instead of struggling with the technology. 
\end{description}
...
\subsubsection{Diagnosis Data Module (\mref{mDDM})}  Pranav
...
\subsubsection{Medicine Data Module (\mref{mMDM})}  Pranav
...
\subsubsection{Classification Module (\mref{mCM})} Gurleen
