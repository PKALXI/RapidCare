...
\texttt{transcribeText} & request : FormData & applicableMedicine : String & FailedResponseError \\
\hline
\end{tabular}
\end{center}
...
\begin{itemize}
\item transition: N/A
\item output: Returns the predicted diagnosis for the patient based on preprocessed input given by \texttt{preProcessData}. FormData is expected to contain the contents of the report generating module as seen in the ``Uses'' sections.
...
\begin{itemize}
\item transition: N/A
\item output: preprocess the request body (FormData) which contain the inputs as specified above (from the ``Uses'' in report generating module) using the preprocess local function then Return the predicted medicine for the patient.
\item exception: InputDimError - The expected request body items were not received or were in the wrong format.
\end{itemize}
